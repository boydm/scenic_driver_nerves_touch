  ...
  def init( viewport, {_,_} = screen_size, config ) do
    ...
    device = case config[:device] do
      device when is_bitstring(device) ->
        ...
        device
      _ ->
        msg = "Scenic.Driver.Nerves.Touch requires a device option to start up\r\n" <>
...
IO.inspect(events)
  ...
  defp ev_abs( {:ev_abs, :abs_mt_tracking_id, -1}, %{
    fingers: fingers,
    slot: slot
  } = state ) do
  ...
  defp ev_abs( {:ev_abs, :abs_mt_tracking_id, id}, %{
    fingers: fingers,
    slot: slot
  } = state ) do
    fingers = fingers
  ...
  # set the x position
  defp ev_abs( {:ev_abs, :abs_mt_position_x, x}, %{
    fingers: fingers,
    slot: slot
  } = state ) do
  ...
  defp ev_abs( {:ev_abs, :abs_mt_position_y, y}, %{
    fingers: fingers,
    slot: slot
  } = state ) do
  ...
  defp ev_abs( {:ev_abs, :abs_mt_pressure, pressure}, %{
    fingers: fingers,
    slot: slot
  } = state ) do
  ...
  defp ev_abs( {:ev_abs, :abs_mt_distance, distance}, %{
    fingers: fingers,
    slot: slot
  } = state ) do
  ...
  defp simulate_mouse( %{slot: 0} = state,
  {:ev_abs, :abs_mt_tracking_id, -1} ) do
    %{state | mouse_event: :mouse_up}
  end

  defp simulate_mouse( %{slot: 0} = state,
  {:ev_abs, :abs_mt_tracking_id, _id} ) do
    %{state | mouse_event: :mouse_down}
  end

  defp simulate_mouse( %{ slot: 0, mouse_event: nil } = state,
  {:ev_abs, :abs_mt_position_x, x} ) do
